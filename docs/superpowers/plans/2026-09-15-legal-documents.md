...
FE dev 서버는 Bash 에서 띄운다 (`preview_start` 는 샌드박스에 막힌다). **포트는 5174 고정**이다 — 3000 은 카카오 지도 키 도메인이 등록되지 않아 지도가 항상 폴백으로 빠진다.

```bash
cd frontend && pnpm dev:alt
```
...
> 5174 에 이미 서버가 떠 있으면 **다른 워크트리 코드일 수 있다.** `lsof` 로 cwd 를
> 확인한다.
