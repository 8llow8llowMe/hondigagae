...
- 세 칸은 **새 해석 규칙을 만들지 않는 선**에서 채운다:
  ...
  - `allowanceScope` — 실측 두 원문("전구역 동반가능" / "일부구역 동반가능")과 **정확히 같을 때만** 옮기고
    나머지는 `UNKNOWN`. `OUTDOOR_ONLY` 는 원천에서 본 적이 없어 내지 않는다. "전구역 동반가능(야외 포함)"
    처럼 섞인 문구를 한쪽으로 좁히지 않는다
...
"동반 정보 있음" 으로 읽힌다. **그래서 행이 없고 상세가 비거나 실패하는 곳은 매 실행 다시 부른다.**
교집합이 상한을 넘으면 그 수만큼 기존 행 갱신 예산이 줄고, 그런 곳이 상한 이상이면 갱신이 멈춘다.
대상이 상한보다 적은 지금(≤ 330 < 350)은 매 실행 전량을 돌아 무해하다.

## 4. 남는 것

- `place.pet_allowance_type` · `allowed_pet_size` 에 이 원천을 반영할지(목록 필터·적합도) — 별도 이슈.
- 위 §3 의 한계 — 교집합이 상한을 넘기 시작하면 "시도 시각" 을 따로 기록하는 장치를 둔다.
- 서킷 `tourapi` 를 KorService2 와 함께 쓴다. 앞 단계에서 서킷이 열리면 이 잡은 `TOUR_API_CIRCUIT_OPEN`
  으로 실패한다(파이프라인 부모 FAILED). 제공처가 같아 지금은 그대로 둔다.
- dev/prod TourAPI 키에 KorPetTourService2 활용신청이 돼 있어야 한다. 없으면 매주 파이프라인 부모가 FAILED 다.
