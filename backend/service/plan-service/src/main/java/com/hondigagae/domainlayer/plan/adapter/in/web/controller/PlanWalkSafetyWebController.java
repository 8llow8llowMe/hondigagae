...
public class PlanWalkSafetyWebController {
    ...
    @Operation(summary = "일정 항목 산책 위험도",
        description = "일정 항목마다 **그 항목의 시각 기준으로** 산책 위험도를 판정합니다. "
            ...
            + "코드는 여섯으로 갈립니다 — PAST_DATE(지난 날짜, 다시 물어도 생기지 않습니다) · "
            ...
            + "NO_FORECAST_AT_TIME(범위 안이지만 **그 시각** 예보를 쓸 수 없었습니다) · "
            + "LOOKUP_FAILED(plan→tour 조회 실패). 재시도 안내는 이 코드로 판단하고 문장을 파싱하지 않습니다.\n\n"
            + "**NO_FORECAST_AT_TIME 은 BEYOND_FORECAST_RANGE 와 다릅니다.** 뒤엣것은 날짜만으로 정해져 그날 모든 항목이 "
            + "같은 답을 받지만(일자 단위로 접어 그려도 됩니다), 앞엣것은 물어본 뒤에야 알 수 있고 "
            + "**같은 날 안에서도 항목마다 갈립니다** — 그날 새벽 항목은 답을 받고 낮 항목은 못 받는 일이 있습니다. "
            + "다만 **NO_FORECAST_AT_TIME 이 곧 '영구히 예보가 없다' 는 뜻은 아닙니다** — tour-service 가 기상 원천 장애를 "
            + "빈 예보로 접어 200 을 주는 경우도 같은 코드로 접히므로, 문장은 원인을 단정하지 않습니다.\n\n"
            + "이 사유만 walkSafetyLevel 이 null 이 아니라 **UNKNOWN 등급으로 옵니다** — tour-service 에 실제로 물어봤고 "
            + "tour-service 가 UNKNOWN 을 답으로 준 값이라 버리지 않습니다. 화면 문구는 unavailableReason 을 쓰세요. "
            + "같은 이유로 이 줄에는 **placeTitle · targetDateTime · basisPetId · petConditionApplied 도 남습니다** — "
            + "다른 사유 줄에서는 비어 있는 값들입니다.\n\n"
            + "petConditionApplied 는 **항목마다** 내려갑니다 — 기준 반려견(basisPetId)이 날짜별로 갈릴 수 있어 일정 단위로 하나만 "
            + "내리면 날이 다른 항목에서 틀립니다. **판정을 못 낸 항목은 원격에 묻지 않았으므로 null 이고**(단 위 NO_FORECAST_AT_TIME 은 예외), "
            + "false(물어봤고 반려견 특성 없이 일반 조건으로 판정함)와 구분됩니다 — falsy 로 뭉뚱그리지 마세요.\n\n"
