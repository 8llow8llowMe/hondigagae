 ...
 *
 * <p><b>쿼터가 희소하다.</b> 개발계정 일 1,000건을 목록(약 17콜)·이 스텝·추가 이미지 스텝이
 * 나눠 쓴다. 그래서 두 겹으로 방어한다 — (1) 실행당 호출 상한(기본 300), (2) "intro 없는 곳
 * 먼저 → synced_at 오래된 순" 증분 선정. 주 1회 실행이 반복되며 전량을 덮고 이후에는 갱신
 * 순환이 된다. 이미지 스텝도 #478 로 같은 규칙을 갖게 되어(상한 400) 두 스텝이 함께 예산 안에 든다.
 *
