 ...
 * <b>정규화 제목 일치 + 좌표 근접</b> 이중 검증을 통과한 경우에만 채운다
 * ({@link PlaceIdentityPolicy#isSamePlaceForImageBackfill}, 반경은
 * {@link PlaceIdentityPolicy#IMAGE_BACKFILL_RADIUS_M}).
...
public class PlaceImageBackfillProcessor {
    ...
    private boolean backfillOne(PlaceImageBackfillTargetQueryResult target, String areaCode) {
        ...
        for (ImportedPlace candidate : candidates) {
            ...
            if (!PlaceIdentityPolicy.isSamePlaceForImageBackfill(target.title(), candidate.title(), distance)) {
                continue;
            }
