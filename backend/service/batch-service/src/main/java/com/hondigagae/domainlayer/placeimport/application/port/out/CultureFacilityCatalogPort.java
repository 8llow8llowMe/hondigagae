 ...
 *
 * <p><b>읽을 파일은 인자로 받는다</b> (#379). 예전에는 어댑터가 설정값 경로 하나를 직접 읽었지만,
 * 지금은 포털에서 받은 임시 파일일 수도 로컬 우회 파일일 수도 있다 - 어느 쪽인지는
 * {@code CultureFacilitySourceProcessor} 가 정하고 이 계약은 경로만 받는다.
 */
public interface CultureFacilityCatalogPort {
     ...
     *
     * @param csvFile 읽을 CSV 파일
     * @param sido    시도 명칭 (예: 제주특별자치도). null 이면 전국
     */
    List<ImportedCultureFacility> readTravelFacilities(Path csvFile, String sido);
     ...
     */
    List<ImportedEmergencyFacility> readEmergencyFacilities(Path csvFile, String sido);
}
