...
public record AuthEmailCodeVerifyRequest(
    ...
    @Schema(description = "인증코드를 받은 이메일 주소", example = "user@example.com", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = AuthValidationMessage.EMAIL_REQUIRED)
    @Email(message = AuthValidationMessage.EMAIL_FORMAT_INVALID)
    String email,

    @Schema(description = "메일로 받은 인증코드", example = "A3K7MP2X", requiredMode = Schema.RequiredMode.REQUIRED)
