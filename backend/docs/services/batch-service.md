...
> 넣고 있어, 재적재하면 약 1,200행에 "실외"라는 근거 없는 값이 새로 박힌다. 그 뒤에는 사후
> UPDATE 가 유일한 길이다. 되돌리는 운영 SQL 과 확인 쿼리는 #753 이 `data-refresh-guide.md` 에
> 추가하는 절에 있다. **코드 머지 순서가 아니라 배치 실행 순서 기준이다** — #726 이 먼저
> 머지돼도 상관없지만, 재적재 실행보다는 #753 이 앞서야 한다.

새로 보이게 되는 약 1,200건은 **기존 행을 건드리지 않는다.** 근거는 식별자 전략이다 —
`PlaceIdFactory` 가 TourAPI 행의 id 를 `contentId` 로 결정하고 고유 키가 `(source, source_key)`
라, 새 콘텐츠는 전부 INSERT 이고 기존 880건은 같은 행에 UPDATE 된다 (`merged_into_id` 는
upsert 가 건드리지 않아 기존 병합이 풀리지 않는다).

1. `placeImportJob` 을 돌린다. 기존 행은 `synced_at` 이 갱신되고 `delisted_at` 은 NULL 로 되살아난다.
2. **delisting 은 안전하다.** 적재(약 2,100) > 직전 활성(약 880) 이라 급감 가드에 걸리지 않고,
...
건수 가드에 걸린 것이다 (`place-import-volume.min-rows` / `max-rows`, 기본 1,200 / 20,000).
하한은 이번 결함(880건)처럼 원천 필터가 조용히 어긋난 경우를, 상한은 지역 필터가 풀려 전국이
들어오는 경우를 잡는다. **기본값은 넓게 잡혀 있다** — 실측 2,124건에는 적재 대상이 아닌 축제(15)
25건이 들어 있고, 적재 대상에 포함된 여행코스(25)는 0건이라 실제 총량은 `2,124 - 25 = 2,099` 다.
이 가드는 **총합**만 보므로 첫 재적재 뒤 실제 건수를 확인해 값을 조인다.
...
**쿼터를 먼저 확인하고 날짜를 고른다.** 대상이 약 964곳에서 약 2,100곳으로 늘면서 목록 호출이
...
**첫 재적재 실측 후 다시 조인다.** 수동 이미지 백필(276콜)과 같은 날에 겹치지 않게 잡는 것이 안전하다.
...
2. **추가 이미지(`placeImageImportStep`, #478)** — detailImage2 를 장소당 1회 불러 place_image 를
   ...
   `place.image_synced_at` 오래된 순 → id** 로 고른다 — 전량(약 2,100곳) 커버는 약 6주 순환이다.
...
전량(각 약 2,100콜)을 돌면 넘친다. 그래서 **두 단계 모두** 실행당 상한 + 증분 대상 선정으로 나눠
