...
| **카카오 로컬(장소 검색)** | 카카오 | 반려견 동반 식당·카페 실시간 검색 | **실시간 호출 전용 — 저장 금지** |
| **전국 반려동물 동반 가능 문화시설** | 한국문화정보원 | 장소 마스터 확장(제주 1,191건), 동물병원 | 파일(CSV) 다운로드 → 배치 적재 |
...
**카카오 로컬 API 응답은 어떤 형태로도 저장하지 않는다.** 카카오 공식 답변 기준으로 DB 영구 저장뿐 아니라
세션 단위 임시 저장과 캐싱도 허용되지 않으며 실시간 호출만 가능하다.

- `place` 등 어떤 테이블에도 넣지 않는다. Redis 캐시도 두지 않는다 (다른 외부 API와 정반대다)
- 요청이 올 때마다 호출해 응답에 그대로 실어 보내고, 카카오맵 상세 링크(`place_url`)로 출처를 표시한다
- 일정에 남길 수 있는 것은 사용자가 직접 고르고 입력한 이름 정도이며 좌표·URL은 저장하지 않는다
- 세부 설계는 `place-data-integration.md` §6·§7 참고

한국문화정보원 문화시설 데이터는 반대로 **파일데이터라 활용신청도 인증키도 필요 없고 저장에 제약이 없다.**
