...
- [ ] 터치 영역이 44px 이상이다 (지도 마커·필터 칩 포함)
- [ ] 색만으로 정보를 전달하지 않는다
...
- [ ] **표면 · 레이아웃 · DOM 순서를 건드렸다면 `pnpm e2e` 도 돌렸다.** `pnpm verify` 에
      **e2e 는 들어 있지 않은데 CI 는 돌린다** — `surface.spec.ts` 가 세로 기준선과 블록
      ...
      `#457` 검사가 스트립을 못 찾았다).
  - **포트를 반드시 비켜 준다 — `E2E_PORT=<빈 포트> pnpm e2e`.** 기본값이 **5175** 인데
    이것은 `pnpm dev:alt2` 와 같은 포트이고, `reuseExistingServer` 가 로컬에서 켜져 있다.
    다른 워크트리 세션이 5175 에 dev 서버를 띄워 뒀으면 **그 세션의 코드를 조용히 검사하고
    통과한다.** 포트를 갈라 둔 `webServer` 주석의 전제(사람은 5174)가 병렬 세션 포트가
    열리면서 깨졌다.
