...
- **긴급 시설의 필터·반경도 URL 이 소유한다** (`lib/url/emergency-filters.ts` ·
  ...
  `?open24Only=true` · `?openNowOnly=true` · `?radius=40000` 이고 기본값은 생략한다.
...
- **지도 뷰에도 필터가 있다** (`place-map-filter-bar.tsx`). **두 줄이다** — 1행은 유형 축
  가로 스크롤러(fade 마스크 + 원형 화살표), 2행은 지역·더보기 시트 트리거다. 데스크톱 패널
  머리와 모바일 시트 머리가 같은 줄을 쓴다 — 예전에는 지도 보기에 필터 컨트롤이 아예 없어
...
| 화면           | 경로         | API                           | 상태                                                                                                                                         |
| -------------- | ------------ | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 주변 긴급 시설 | `/emergency` | `GET /emergencies/facilities` | **구현** (#13) · **지도 구현** (#14) · **지도 기본** (#353) · **조건 URL 소유** (#371) · **목록 필터 레일 2단** (#419) · **3층 표면** (#460) |
