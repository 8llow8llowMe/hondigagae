...
- **지도 뷰에도 필터가 있다** (`place-map-filter-bar.tsx`). 유형 축을 가로 토글로 펼치고
  지역·동반·실내·크기는 시트로 남긴다. 데스크톱 패널 머리와 모바일 시트 머리가 같은 줄을 쓴다 —
  예전에는 지도 보기에 필터 컨트롤이 아예 없어 조건을 좁히려면 목록으로 되돌아가야 했다.
