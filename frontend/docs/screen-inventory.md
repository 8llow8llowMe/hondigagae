...
- **`hourly` 가 0칸인 것은 "판정 없음"이지 "위험"이 아니다** ([#204](https://github.com/8llow8llowMe/hondigagae/issues/204)).
  판정 자리의 상태는 **넷**이다 (#204 · [#262](https://github.com/8llow8llowMe/hondigagae/issues/262)):

  | 응답                                             | 화면                   | 뜻                                  |
  | ------------------------------------------------ | ---------------------- | ----------------------------------- |
  | `goldenStart` 있음                               | 추천 구간              | 이때 나가면 된다                    |
  | `hourly` 있고 `goldenStart` 없음                 | 위험 단정              | **판정**: 남은 시간이 전부 위험이다 |
  | `hourly` 비었고 `forecastCoverage` `UNAVAILABLE` | 예보 없음 + **재시도** | **장애**: 날씨를 못 받았다          |
  | `hourly` 비었고 그 밖                            | 예보 없음              | **근거 없음**: 판정할 예보가 없다   |
