...
- **`data.temperature` 를 쓰지 않는다.** 그 시각의 기온인데 장소 상세의 기온 라벨은 둘 다
  **하루 단위**라(`detailFeelsLikeTemperature`·`detailMaxTemperature`), 붙이면 시각 기온을
  일 최고로 말하게 된다.
- **`heatIndexCelsius` 에 라벨이 없다** — 위 두 라벨이 하루 단위라 붙일 것이 없었기 때문인데,
  [#253](https://github.com/8llow8llowMe/hondigagae/issues/253) 이 적합도 쪽을 `체감온도`(하루 최대)로
  바꾸면서 **같은 레일에 기준이 다른 열지수 두 개**가 서게 됐다(하루 최대 33.4 · 14:00 시각 32.4).
  지금은 `오늘 이 지역` / `지금 산책` 두 제목과 `{time} 기준` 각주가 갈라 주고 있지만,
  **시각 단위 라벨을 따로 두는 것이 낫다** — 별도 이슈로 다룬다.
...
- **`hourly` 가 0칸인 것은 "판정 없음"이지 "위험"이 아니다** ([#204](https://github.com/8llow8llowMe/hondigagae/issues/204)).
  ...
  | 응답                             | 화면      | 뜻                                  |
  | -------------------------------- | --------- | ----------------------------------- |
  | `goldenStart` 있음               | 추천 구간 | 이때 나가면 된다                    |
  | `hourly` 있고 `goldenStart` 없음 | 위험 단정 | **판정**: 남은 시간이 전부 위험이다 |
  | `hourly` 비었음                  | 예보 없음 | **근거 없음**: 판정할 예보가 없다   |

  **이 절이 한동안 앞의 두 줄만 적어 둬서** 화면이 `goldenStart` 만 보고 갈라졌고, dev
  23:17 KST 에 `hourly: []` 를 받아 "남은 시간이 모두 위험 등급이에요"를 단정하면서 바로
  아래 곡선 자리에서는 "오늘 남은 예보가 없어요"가 나왔다 — 한 카드 안에 모순된 두 문장이
  같이 나갔다. **모르는 것을 나쁜 것으로 말하지 않는다** (루트 `CLAUDE.md`).
  예보 없음은 **위험 톤을 쓰지 않는다** (DESIGN.md §2-3 — 등급이 없는 자리에 등급 색을
  쓰면 안 된다).

- **`hasGolden` 을 `hourly` 보다 먼저 본다.** 곡선이 비었는지로 먼저 갈라 버리면 서버가
  구간을 주는데 곡선만 못 받은 경우에 실제 추천을 감춘다. 예보 없음이 밀어내야 하는 것은
  **위험 단정 하나뿐**이다.
- **좌표는 `getCurrentPosition()` 이 준다** (#180 · `/emergency` 와 같은 함수). 거부·타임아웃·
...
| 생성 대기·결과 | `/ai-plans/jobs/[jobId]` | `GET .../jobs/{jobId}/stream` **SSE** (+ 폴링 폴백) | **구현 완료** (#84 · SSE #91)        |
...
- **이유 문장에 `[반려견 2]` 꼴 접두가 섞여 온다.** 합집합이라 흐려지는 근거를 서버가 이
  접두로 되살린다 — 화면이 이유를 손대거나 접두를 떼면 그 단서가 사라진다. 접두의 번호는
  `petIds` 순서이고 아이 이름이 아니다.
