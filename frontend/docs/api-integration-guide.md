...
토큰이 500 을 주던 갈래도 같은 PR 에서 401 `SECURITY_003` 으로 닫혔다.
...
| 산책 코스        | **5분**     | 30분     | 2       | 제주올레 29코스. batch 적재라 세션 중 불변 — 장소와 같은 값·같은 이유 |
| **AI job 상태**  | **0**       | 1분      | **0**   | 폴링(`refetchInterval`)이 신선도를 담당. retry는 폴링과 중복          |

**해석**

- 산책 코스에 장소와 같은 값을 준 이유: 제주올레 코스도 배치(`walkcourseimport`)로 적재되어 세션 중에 바뀌지 않는다.
  걷는 시간·정렬을 왕복하며 만지는 화면이라 재조회를 줄이는 것이 체감에 직결된다. `retry` 는 전역 기본값 그대로다 (#618).
