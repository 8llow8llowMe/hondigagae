...
- **코드를 가리킬 때 행 번호를 쓰지 않는다. 파일 + 심볼 이름으로 지칭한다** (아래).

## 코드 지칭 규약 — 행 번호를 쓰지 않는다 (#849 · #861)

문서가 `파일:행` 으로 코드를 가리키면 **구조적으로 낡는다.** 정본 근거는
`features/공통/등급배지-축라벨-세부명세.md` D5-3 이다.

- **`[FE]` 커밋이 뒤따르는 `[DOCS]` 커밋의 측정 대상을 민다.** 주석 한 줄만 더해도 아래 모든 행
  번호가 틀린다.
- **틀린 행 번호는 조용하다.** 파일이 살아 있는 한 링크는 늘 "유효해 보인다" — 기계 검사를 둘
  돌려 봐도(파일 없음 / EOF 초과) **둘 다 0건**이다. 읽는 사람만 엉뚱한 줄을 본다.
- **심볼 이름은 그 자체가 검색어다.** 파일이 바뀌어도 따라가고, 사라지면 `grep` 이 0건을 내어
  **틀렸다는 사실이 드러난다.**

```text
쓰지 않는다:  `lib/plan/day-items.ts:86` 의 되싣기
이렇게 적는다: `lib/plan/day-items.ts` `toEditItems` 의 되싣기
```

- 심볼은 함수·컴포넌트·상수·테스트(`it` / `describe`) 이름. CSS 는 선택자 이름.
- 한 파일에 같은 이름이 여럿이면 **갈래 이름을 덧붙여** 가른다 (`PlaceInsightRow` 좁은 폭 /
  넓은 폭). 그것도 모호하면 그때는 함수를 쪼갤 때이지 행 번호를 되살릴 때가 아니다.
- **예외는 "밀렸다는 증거"뿐이다.** `place-congestion-panel.tsx:218 → 326` 처럼 드리프트를
  기록한 `A → B` 꼴은 옛 숫자가 있어야 뜻이 산다.
- **날짜 붙은 스냅숏**(`superpowers/specs/*` · `superpowers/plans/*`)은 그때의 코드를 가리키는
  기록이라 이 규약의 대상이 아니다.
