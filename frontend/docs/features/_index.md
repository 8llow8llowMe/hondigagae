...
| pet     | `공통명세.md`, `목록/등록/수정-세부명세.md`                                                                     | 구현 완료                                     | 구현                 | 등록 상한 5마리 / **사진·체중·대표견 FE 미연동** (#126)               |
...
**`petWeightKg` 는 아직 쓰지 않지만 이유가 바뀌었다.** 처음에는 `Pet` 프로필에 체중 필드가
없어서였는데, **백엔드가 PR #107 로 `weightKg` 를 추가했다.** 이제는 FE 폼이 그것을 받지
않을 뿐이다 — 프로필에 체중을 넣는 순간 이 필터 축이 함께 열린다
([#126](https://github.com/8llow8llowMe/hondigagae/issues/126)). 지금은 크기(`sizeType`)만 보낸다.
