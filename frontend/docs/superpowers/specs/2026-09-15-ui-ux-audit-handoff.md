...
| #635 | #649 | `/about` 8절 소개 페이지 — 질문 3개 구조 · 스크롤 등장 · 예시 4개 재생 (AB-1)                                 | develop |
...
| `/about` 재작성 뒤에도 약관 절(`LEGAL_LINKS`)을 남긴다                       | 푸터는 768 미만 은닉 · 마이페이지는 로그인 필요 · `(auth)` 는 푸터 없음 — 셋이 동시에 참이라 이 절이 빠지면 **가입 전 모바일에서 약관을 읽을 수단이 사라진다**. #649 리베이스에서 삼켜진 것을 되돌렸다 |
...
**이슈 7개가 열려 있다 (#650~#656).** 표의 `#` 열은 진단 문서 §4 의 일련번호다.

| #    | 이슈 | 이슈 제목                                                                                 | 진단 ID                   | 주로 고치는 곳                                                                               | 시안      |
| ---- | ---- | ----------------------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------- | --------- |
| 6    | #650 | `[FE] feat: 장소 상세 모바일 상단에 판정 요약 3줄을 둔다`                                 | D-1                       | `features/place/**` 상세 뷰, 갤러리 높이 200                                                 | §2        |
| 7    | #651 | `[FE] feat: 혼잡도 집중률을 정수·상대 표현으로 바꾼다`                                    | D-3                       | `place-congestion-panel.tsx`                                                                 | §2 ④      |
| 8    | #652 | `[FE] feat: 등급 배지에 축 라벨을 붙여 혼잡도와 적합도를 가른다`                          | G-1 · D-2                 | `components/metric.tsx` `MetricBadge` + 호출부. **서버 enum name 은 그대로, 앞에 축 라벨만** | §2 ② · §6 |
| 9    | #653 | `[FE] refactor: 일정 상세 헤더에서 상태 변경을 메뉴로 내리고 준비물을 일차 아래로 옮긴다` | PL-2 · PL-3 · PL-4 · PL-5 | `features/plan/**` 상세 (`plan-status-action.tsx` 는 develop 에서 최근 바뀜 — 먼저 읽을 것)  | §4        |
| 10   | #654 | `[FE] feat: 긴급 시설 필터를 진료중 우선으로 재배열하고 영업시간을 오늘 한 줄로 접는다`   | E-3 · E-4                 | `emergency-filter-fields.tsx` · `facility-row.tsx`                                           | §3 ③ ④    |
| 10-b | #655 | `[FE] feat: 긴급 시설 지도 클러스터 알약을 숫자 원형 마커로 바꾼다`                       | E-1 후반                  | `emergency-map-*` — **`fe-map-reviewer` 검토 필수**                                          | §3 ①      |
| 10-c | #656 | `[FE] fix: 골든타임 곡선의 추천 구간 면을 시각별 등급으로 칠한다`                         | H-2 잔여                  | `walk-times-section.tsx` `HourlyCurve` (홈-세부명세 D4-1-c 갱신)                             | —         |

> **B-6 범위 정정.** 진단 문서는 이 레인을 `D-1 · D-4` 로 묶었지만 **D-4(하단 고정 액션 바)는 이미 구현돼 있다** — `place-detail-section.tsx` 의 `PlaceDetailActionBar` 가 모바일 `sticky bottom-16 lg:hidden`, 데스크톱 좌측 레일 `inset="card" hidden lg:block` 으로 이미 두 곳에 그려지고 미로그인 분기도 있다. #650 은 **D-1 만** 한다.
...
소셜 로그인 우선 배치(A-1 · A-2, §8) · 하단 탭 응급 5탭(G-3) · 숫자 정수화(G-2: `26.0℃`→`26℃`) · ~~소개 페이지 3단(AB-1)~~ **#635 / PR #649 로 완료** · 헤더 로그인·회원가입 라벨 message 키 이전 · 마이페이지 로그아웃 위계(M-1) · 반려견 삭제 버튼 위계(PT-3) · 일정 예산 통화 접미(PL-6) · 저장 한도 안내 조건(F-2).
