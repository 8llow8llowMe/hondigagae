...
| 컴포넌트                                           | 위치                               | 비고                                                                                                                                                                                                                                                                      |
| -------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Button`                                           | `src/components/button.tsx`        | variant: primary / secondary / ghost / danger / dangerOutline / kakao / inverse / inverseOutline, size: sm / md / lg. **화면에 놓인 삭제 버튼은 `dangerOutline`** (채움은 확인 다이얼로그 전용). `inverse`/`inverseOutline` 은 소개 페이지 그린 밴드 전용(DESIGN.md §0-2) |
...
- **목록 화면의 우측 열은 카드 하나다** (#556). 제목 · 부제 · 검색 · 필터 칩 · 보기 토글이
  ...
  이상에서 **머리는 고정되고 본문(목록)만 구른다** — 레일이 뷰포트보다 길어 페이지가 구를
  때도 그렇다. 그 열이 `position: sticky` 로 접힘 안에 남기 때문이다 (#598, globals.css 의
  `.list-column`). 도구를 카드 밖에 두던 규칙이 사라진 것이
...
- **터치 영역 44×44px 은 지도 위 타깃(마커·묶음 원)에만 요구한다** (`DESIGN.md` §7, #883).
  그 밖의 모바일 컨트롤에는 44 하한이 없고, 지도 화면의 칩은 오히려 모바일 36(`h-9`)이다 —
  375 실화면에서 44 칩이 세 줄로 쌓여 시트 본문을 먹었다. **기존 `h-11` 사용처는 화면별
  후속으로 옮긴다** (규칙이 바뀐 것이지 기존 화면이 무효가 된 것은 아니다).
