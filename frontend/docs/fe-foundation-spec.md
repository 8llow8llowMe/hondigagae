...
| F   | **폼 규약** (`docs/form-guide.md`)               | **해소** (2026-08-27)    | zod + 자체 경량 훅. 백엔드 `ValidationErrorBody` 실측 반영 — 필드별 **첫 오류만** 채택               |
...
### Phase 4.5 — 디자인 리뷰와 수정 (2026-08-26)

`fe-design-reviewer` 를 처음 실제로 돌렸다. **발견 15건 중 7건을 수정**했다. 지적은 전부 독립 검증했다.

| #      | 발견                                                                                                                                                           | 조치                                                                           |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 1      | **`cn()` 의 tailwind-merge 가 커스텀 타이포 토큰을 글자 색으로 오인해 삭제** — 배지 색 매핑이 전부 죽어 모든 배지가 검정으로 렌더되고 칩이 14px 대신 16px 였다 | `extendTailwindMerge` 로 스케일 등록 + 회귀 테스트 10개                        |
| 2      | 375에서 카드 제목이 가로로 넘쳐 잘림 (`keep-all` 이라 공백 없는 긴 장소명이 안 끊긴다)                                                                         | 제목에 `break-words`                                                           |
| 4      | 상태 화면의 유일한 액션 버튼이 32px (44px 규칙 위반)                                                                                                           | `size="md"`                                                                    |
| 5      | 칩 focus ring 상단 4px 이 `overflow-x-auto` 에 잘림                                                                                                            | `py-1 -my-1`                                                                   |
| 8·9·14 | **대비 미달** — 배지 6종 중 4종(warn 2.35 / accent 3.33 / danger 3.83 / info 3.86), 에러 제목 4.38, 캡션 3.20                                                  | tint 위 텍스트 전용 `-700` 4종 신설, `danger-500` 보정, 캡션을 `--fg-muted` 로 |

**검증**: 배지 6종 + 에러 제목 + 캡션 = 8개 조합 전부 4.5:1 이상. 브라우저 실측으로 색 복구 확인.

**교훈**: `component-guide.md` §3 에 "`className` 은 `cn()` 으로 병합한다"는 규약을 써놓고
그 `cn()` 이 토큰을 지우고 있었다. **lint 는 arbitrary value 만 막지 병합 결과는 보지 못한다.**
유틸리티 함수 자체에 회귀 테스트가 필요하다.

**부수 수확**: Phase 4 에서 못 밝힌 "스켈레톤이 영원히 안 넘어감" 의 원인이 밝혀졌다 —
브라우저 페인 탭이 `visibilityState: hidden` 이라 React Query 가 `fetchStatus: 'paused'` 로
멈춘 것이었다. 제품 결함이 아니라 도구 환경 문제다.

### Phase 5 — 계약 동기화와 mock 계층 (2026-08-26)

`origin/develop` 이 7커밋 앞서 있었고 장소 API 가 크게 바뀌어 FE 계약이 낡아 있었다.

**신규 API 2종** — `GET /places/nearby`(좌표 반경 검색), `GET /emergencies/facilities`(동물병원·약국).
긴급 시설은 `screen-inventory.md` 에 "미착수"로 적혀 있던 기능이라 **착수 가능 범위가 넓어졌다.**

**장소 필터 확장** — `indoor` / `allowedPetSize` / `sourceCategory`.
`PlaceItem` 에도 `indoor` / `sourceCategory` / `sourceName` 이 붙었다.
`indoor` 는 원천에 정보가 없으면(`null`) **true/false 어느 쪽 필터에도 잡히지 않는다.**

**mock 계층** — BFF·serverFetch 에서만 분기해 클라이언트가 mock 존재를 모르게 했다.
`MOCK_API=true` 일 때만, 프로덕션 빌드에서는 항상 비활성. 백엔드 동작(필터·커서·size 400·404)을
흉내 내고, 처리 대상이 아닌 경로는 실제 게이트웨이로 넘긴다.
허용 기준은 `api-integration-guide.md` §9 에 명문화했다 —
**"없는 API 를 상상해서 채우는 것"은 여전히 금지, "실재하는 계약 기반 개발용 fixture"는 허용.**

**mock 으로 해소된 것**

| 항목                             | 결과                                                                                              |
| -------------------------------- | ------------------------------------------------------------------------------------------------- |
| `break-words` 제목 오버플로 수정 | 실데이터로 검증 — `제주특별자치도립김창열미술관` 2줄 wrap, 가로 오버플로 0건                      |
| 배지 색 매핑                     | 4종이 실제로 구분되는 것을 화면에서 확인                                                          |
| skeleton 높이 (디자인 리뷰 #10)  | 실데이터 카드가 114~162px 로 흩어져 고정 `h-28`(112px)과 어긋났다 → **구조형 skeleton** 으로 교체 |
| nullable 숨김                    | 주소·전화 없는 카드에서 해당 줄이 숨겨짐                                                          |
| 빈 결과                          | 재시도 버튼 없이 필터 초기화만 제공됨                                                             |

**발견해 이슈 등록**: `facilityId` 가 `long` 이고 예시값 `4611686018427387904` 가
`Number.MAX_SAFE_INTEGER` 를 약 500배 초과한다. `JSON.parse` 단계에서 손상되므로 FE 로는 막을 수 없다.
같은 서비스의 `placeId` 는 `String` 으로 내려주고 있어 `facilityId` 만 예외다 → [#8](https://github.com/8llow8llowMe/hondigagae/issues/8)

### 미결 (Phase 4 이후)

| 항목                                                                                    | 상태                                                                                                                                    |
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **T4 폼 규약** (`docs/form-guide.md`)                                                   | **작성 완료** (2026-08-27) — zod + 자체 경량 훅. 로그인·회원가입 화면에서 최초 적용, 반려견 프로필(#12)에서 복잡 폼으로 검증            |
...
| ~~디자인 리뷰 UX 3건~~                                                                  | **해소** — 요약 줄 + 선택 칩 자동 센터링(#3), 반응형 밀도(#6), 스크롤바 숨김 + 방향 인지 fade(#7). 패턴은 `styling-guide.md` §4 에 기록 |
| 디자인 리뷰 나머지 3건                                                                  | `prefers-reduced-motion` 0건, 문구 어미 혼용, 홈에 링크·nav 없음 (skeleton 높이는 구조형 skeleton 으로 해소)                            |
| ~~`PlaceCard` 실데이터 시각 검토~~                                                      | **해소** — mock 계층으로 확인했다. 단 `firstImage` 가 있는 경우(외부 이미지 로딩)는 여전히 미검증                                       |
| BE 이슈 [#8](https://github.com/8llow8llowMe/hondigagae/issues/8) — `facilityId` 정밀도 | **등록됨.** 긴급 시설 화면 구현 **전에** 해결되면 FE 영향이 없다                                                                        |
