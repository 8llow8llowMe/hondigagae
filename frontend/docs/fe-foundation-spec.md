...
| **디자인 리뷰 UX 3건**                           | 미수정 — 선택 칩이 375에서 화면 밖(필터 적용 사실 불가시), 태블릿·데스크톱 밀도가 모바일 그대로, 스크롤바가 밑줄로 오독. **제품 판단 필요해 보류** |
| 디자인 리뷰 나머지 4건                           | skeleton 높이(실데이터 후 재측정), `prefers-reduced-motion` 0건, 문구 어미 혼용, 홈에 링크·nav 없음                                                |
| `PlaceCard` 실데이터 시각 검토                   | 미완 — 백엔드 없이 fixture 주입으로만 측정. `firstImage` 있는 경우 미검증                                                                          |
