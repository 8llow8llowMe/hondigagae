...
| 지도 뷰            | 목록 캐시 재사용             | client                    | **첫 화면은 별도 조회 금지.** `이 지역에서 재검색` 을 누를 때만 `/places/nearby` (#14 · #396)                                                                                                                                                                                                                |
...
| 제주올레 코스 목록 | **서버 프리페치**            | client `useQuery`         | 공개 화면 → SEO. **커서가 없다**(29개 전량). 활동량 기본값은 **대표견**이라 서버가 `GET /members/me/pets` 를 함께 부른다 — nav 선택견은 `localStorage` 라 서버가 못 읽어 첫 렌더 뒤 29 → 6 으로 줄어드는 것이 보인다 (#618)                                                                                  |
| 제주올레 코스 상세 | **서버 프리페치**            | client                    | 공개 화면. 골든타임은 client 이고 **좌표가 있는 코스에서만** 부른다 (25/29 가 `lat`/`lng` null). `searchParams` 는 조회가 아니라 **되돌림 링크용**이다 — 그 쿼리가 URL 을 가르므로 `canonical` 을 못박는다 (#783)                                                                                            |
