...
| 산책 코스 상세   | **서버 프리페치**            | client                    | 공개 화면. 골든타임은 client 이고 **좌표가 있는 코스에서만** 부른다 (25/29 가 `lat`/`lng` null). `searchParams` 는 조회가 아니라 **되돌림 링크용**이다 — 그 쿼리가 URL 을 가르므로 `canonical` 을 못박는다 (#783)                                                                                            |
