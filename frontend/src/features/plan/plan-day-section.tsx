      ...
      <div className={INSET_CLASS.card}>
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 pt-5">
          {/*
            ...
            **값이 저장소 공통의 `scroll-mt-20`(80) 이 아니라 28(112) 이다.** 다른 앵커들은
            제목이 곧 블록의 위 끝이라 80 이면 되는데, 이 `h2` 는 카드 위 테두리보다
            **30px 아래**다 — `pt-5`(20) + 카드 테두리 + 제목 줄의 위 leading. 그래서 뛴
            뒤 카드 상단은 늘 `scroll-mt - 30` 에 선다.

            **30 은 실측값이다** (768 · 2026-09-22). 처음에는 `pt-5` 만 보고 20 으로 잡아
            `scroll-mt-24`(96)를 줬는데, 그러면 카드 상단이 66 에 서서 `md:h-16`(64) 헤더와
            **2px** 밖에 안 벌어졌다 — 가려지지는 않지만 카드가 헤더에 붙어 뛴 결과가
            "카드 중간에 떨어진" 것처럼 읽힌다. 112 면 카드 상단이 82 라 모바일(56)에서
            26px · 태블릿 이상(64)에서 18px 벌어진다.
          */}
          <h2 id={anchorId} className="text-title-1 text-fg scroll-mt-28 font-bold">
