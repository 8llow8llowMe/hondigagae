 ...
 * 목록이 된다. **308 은 줄만 센 값이다** — 구획선 · 섹션 머리 · `mb-1` 까지 더한 `nav`
 * 실측은 3줄 173px · 7줄 353px 이다 (1024 폭 2026-09-22 · D18-2). 예산을 잴 때는 353 쪽을
 * 쓴다. **여행은 최대 30일이다** (`PLAN_PERIOD_MAX_DAYS`) — 넘치는 일자는 개수로만
 * 말하고 판정 자체는 아래 일자 카드가 그대로 갖는다.
 *
 * **이 상한이 세는 것은 판정이 있는 일자다** (#847) — 꼬리의 `정보 없음` 은 줄을 쓰기
...
/**
 * 꼬리의 연속 `정보 없음` 을 뗀 목록 (#847).
 *
 * ## 왜 꼬리만인가
 *
 * 목차는 **"목차이면서 요약"** 이다 (D18-2). 꼬리의 `정보 없음` 은 둘 다 아니다 — 요약할
 * 판정이 없고, 앵커로 가 봐야 일자 카드도 같은 사유 문장 하나를 낸다. 그 줄들은 `외 N일`
 * 이 개수로 말하는 편이 짧고 정확하다.
 *
 * **앞·중간의 `정보 없음` 은 남긴다.** 거기서는 그것이 **그 날의 사실**이다 — 판정이 난
...
function PlanVerdictToc({ verdicts }: { verdicts: PlanDayWeatherItem[] }) {
  ...
  /*
    ...
    **`정보 없음` 줄을 일곱 개 세우는 것보다 짧다** — 353px 이 두 줄로 줄어든다 (D18-2).
  */
  if (shown.length === 0) {
    return (
