...
export type OAuthNextAction =
  ...
  | 'consent'
  /**
   * **우리 쪽** 가입 동의가 비었다 — 동의 화면에서 체크한 뒤 다시 시도해야 한다 (#688).
   *
   * `consent` 와 갈라 두는 이유는 고칠 자리가 다르기 때문이다: 저쪽은 제공자의 동의
   * 화면이고, 이쪽은 우리 체크박스다. 로그인 화면의 소셜 버튼은 동의를 싣지 않으므로
   * (기존 회원 전용) 신규 사용자가 거기로 들어오면 반드시 여기로 온다.
   *
   * 목적지는 `/signup/social/{provider}` — 동의 3종과 들어온 제공자 버튼 하나만 있는
   * 전용 화면이다 (#707). 계산은 `oauth-callback-view.tsx` 의 `signupConsentPath` 다.
   */
  | 'signup-consent'
