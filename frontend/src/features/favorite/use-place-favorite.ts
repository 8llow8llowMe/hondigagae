 ...
 *
 * **저장 여부를 목록에서 읽는다.** 저장·해제 응답에 본문이 없고(`Response<Void>`) 상한이
 * 100곳이라 목록 전량이 가볍다. 저장한 장소 화면(#127)이 **같은 캐시를 쓴다** — 거기서
 * 해제하고 상세로 돌아오면 하단 바가 이미 맞다.
 *
 * 단건 조회(`GET /favorites/places/{placeId}`)는 백엔드에 생겼지만 쓰지 않는다 —
 * 이유는 `queries.ts` 주석.
 *
