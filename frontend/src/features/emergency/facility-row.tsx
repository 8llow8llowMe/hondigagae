 ...
 * 정하고 버튼은 그 안에 든다. **44 를 밑도는 값을 의도적으로 고른 자리**였고, #883 이
 * §7 의 44 하한을 지도 타깃으로 좁히면서 이제 규칙과 어긋나지도 않는다 — 근거는 그때나
 * 지금이나 같다: 지도 패널의 `DirectionsLink`(전폭 44px)가 같은 행동의 큰 과녁을 계속
 * 들고 있고, 목록 행은 40px 두 개를 8px 떼어 오조작 쪽을 막는다.
 */
