...
export function EmergencyMapView({ listHref, mapHref }: { listHref: string; mapHref: string }) {
  ...
  */
  ...
  /*
    ...
    **카카오는 이 애니메이션 이동에서도 `idle` 을 낸다** — `bounds` 가 좁아진 레벨 4
    ...
    얼림은 선택을 풀어도 남았다가 사용자가 지도를 옮기면 풀린다(아래 `userMovingRef`).
  */
  ...
  */
  ...
  /*
    **선택을 풀어도 얼린 영역은 그대로 둔다** (#933).
    ...
    예전에는 선택이 풀리는 순간 `frozenBounds` 를 비웠다. 그런데 그때 `bounds` 는 이미
    선택-확대가 낸 `idle` 로 **레벨 4 프레임**이 돼 있다(위 `frozenBounds` 설명의
    2026-09-23 실측) — 그래서 고른 행을 다시 눌러 해제하면 목록이 114곳에서 "지도에
    보이는 12곳" 으로, 밀도가 낮은 곳에서는 **고른 그 한 곳만** 남았다(2026-09-25 실측).
    해제는 "그만 볼게" 지 "지금 확대된 프레임만 보여줘" 가 아니다.

    얼림은 이제 **사용자가 지도를 직접 옮긴 뒤**에 풀린다 — `MapCanvas.onUserMove`
    (끌기·휠·핀치·더블클릭) 가 온 뒤의 첫 `idle` 에서(`handleBounds`), 또는 고른 동안
    옮겼다면 해제하는 그 순간(`handleSelect`). `idle` 만으로는 가를 수 없다: 선택-확대가
    내는 `idle` 은 이동 거리에 따라 300ms ~ 1.3s 뒤에 와서(2026-09-25 실측), 시간으로
    잘라 두면 멀리 이어 고른 행에서 그 `idle` 이 사용자의 이동으로 세어졌다.

    반경·위치가 바뀌면 여전히 즉시 푼다 — 아래 해제 effect.
  */
  /** 사용자가 옮기기 시작했다 — 다음 `idle` 이 그 이동이 끝난 영역이다 */
  const userMovingRef = useRef(false)
  /** 얼린 뒤 사용자가 지도를 옮겼는가 — 해제·다음 선택이 새 `bounds` 를 쓸지 정한다 */
  const frameMovedRef = useRef(false)
  /** `handleBounds` 는 `[]` 의존성이라 지금 선택을 ref 로 읽는다 */
  ...
  */
  ...
  /*
    **선택은 그것이 딛고 있던 전제가 깨지면 놓아준다** — 하나의 규칙으로 두 리뷰
    ...
    표로 고정한다). 셋 중 하나라도 깨지면 선택을 푼다. **얼림은 반경·위치가 바뀐
    갈래에서만 같이 푼다** (#933) — 필터로 고른 시설이 빠진 것은 영역이 바뀐 것이
    아니라서, 거기서 얼림을 풀면 목록이 확대된 프레임으로 줄어드는 결함이 재발한다.
    선택을 푼 뒤에도 얼림이 남아 있을 수 있으므로(위 `userMovingRef` 설명)
    반경·위치 검사는 `frozenBounds` 가 살아 있는 동안에도 돈다.
    ...
    **무한 루프가 안 나는 이유.** 가드가 "선택도 얼림도 없으면 return" 이다 — 반경·위치
    갈래는 둘을 함께 비우므로 다음 렌더에서 즉시 return 하고, 시설이 빠진 갈래는
    `selectedId` 만 비워 다음 렌더부터는 반경·위치 검사만 남는다(그 값이 anchor 와
    같으면 아무것도 안 한다). 새로 고르면 `handleSelect` 가 anchor 를 다시 채우므로
    같은 판정이 그 새 선택에 대해 처음부터 다시 시작된다.
  ...
  */
  useEffect(() => {
    // 선택도 얼림도 없으면 검사할 전제가 없다 — 루프 방지
    if (selectedId === null && frozenBounds === null) return

    const anchor = selectionAnchorRef.current
    if (anchor === null) return

    const areaChanged = anchor.radius !== board.radius || anchor.position !== board.position
    if (areaChanged) {
      setSelectedId(null)
  ...
  }, [selectedId, frozenBounds, board.radius, board.position, visible])
  ...
  */
  ...
  const handleBounds = useCallback((next: MapBounds) => {
    ...
    // 우리가 건 이동(선택-확대 등)의 `idle` 은 얼림을 건드리지 않는다
    if (!userMovingRef.current) return
    userMovingRef.current = false

    // 사용자가 옮겼다. 고른 동안이면 표시만 해 두고(해제·다음 선택이 읽는다), 풀린 뒤면 얼림을 푼다
    if (selectedIdRef.current === null) setFrozenBounds(null)
    else frameMovedRef.current = true
  }, [])
  ...
  /*
    ...
    **선택을 토글로 만든다 — 이미 고른 것을 다시 누르면 해제한다.** 닫기 버튼이 있던
    플로팅 선택 카드는 이미 삭제됐다 — 토글이 없으면 한 번 고른 뒤로는 세션 내내
    목록이 얼어붙는다(브라우저 실측 — 캡션이 `목록 136곳` 에 그대로 머무름).
    **해제가 얼림을 곧장 풀지는 않는다** (위 `userMovingRef` 설명) — 해제 뒤
    사용자가 지도를 옮겨야 풀린다. `MapCanvas.onBoundsChange` 의 `userMoved` 로 그
    "옮겼다" 를 가를 수 없는 것은, 그 값이 선택-확대가 낸 idle 을 포함해 **첫 idle 이후
    전부** `true` 이기 때문이다 — 그래서 입력 쪽 신호(`onUserMove`)를 따로 받는다.
  ...
  */
  const handleSelect = useCallback(
    (id: string) => {
      if (selectedId === id) {
        // 해제 — 얼림은 남긴다. 고른 동안 사용자가 지도를 옮겼을 때만 여기서 푼다:
        // 그때 `bounds` 는 사용자가 맞춘 프레임이다 (위 `userMovingRef` 설명).
        // `boundsStale` 은 여기서 건드리지 않는다 — 지도가 실제로는 안 움직였으니
        // (바로 위 설명) `bounds` 가 그 사이 최신이 됐을 리 없다.
        setSelectedId(null)
        if (frameMovedRef.current) setFrozenBounds(null)
        frameMovedRef.current = false
        return
      }

      setSelectedId(id)
      // 이어 고르면 앞 선택이 얼린 영역을 물려받는다 — 지금 `bounds` 는 앞 선택이 확대한
      // 레벨 4 프레임이라, 그걸로 다시 얼리면 목록이 거기서 줄어든다. 사용자가 그사이
      // 옮겼으면 옮긴 프레임으로 얼린다.
      setFrozenBounds((frozen) => (frozen === null || frameMovedRef.current ? bounds : frozen))
      frameMovedRef.current = false
      setBoundsStale(true)
  ...
  */
  // 얼린 동안은 목록이 지금 프레임이 아니다 — 선택을 풀어도 얼림이 남는다 (위 `userMovingRef`)
  const hideViewportClaim = selectedId !== null || frozenBounds !== null || boundsStale
