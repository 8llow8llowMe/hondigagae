 ...
 * 금지"), 사용자가 **"이 지역에서 재검색" 을 눌렀을 때만** 이 조회가 켜진다.
 *
 * **지도를 옮긴 것만으로는 켜지지 않는다** (#396 의 규칙을 이 화면에도 들였다). 예전에는
 * `idle` 마다 켜졌고, 그 전제가 **한 곳을 골라 확대하는 조작**에서 깨졌다 — 확대는 우리가
 * 그 핀으로 옮겨 준 결과인데 그때마다 좁아진 프레임으로 목록이 다시 조회돼 방금 보던
 * 결과가 사라졌다.
 *
 * 좌표를 query key 에 그대로 넣으면 1m 만 움직여도 캐시가 갈린다. 이제 호출부가 넘기는
 * center 는 **버튼을 누른 자리**(`searchedBounds`)뿐이라 스스로 안정적이다.
 */
