          ...
          */}
          ...
          {/*
            ...
            **좌측 판정 레일에 있었다.** 거기서는 "오늘 이 개에게 적합한가"(적합도) ·
            ...
            그런데 이 카드만 **시간 축이 다르다** — 앞 둘은 `지금` 을 말하고 이것은
            `언제 올까` 를 말한다. 레일은 폭이 400 이라 30일 레일이 설 자리도 아니었다.

            제목 다음 자리로 옮긴다. 장소 이름을 읽은 사람의 다음 질문이 "언제 가지" 이고,
            그 답이 곧 이 서비스의 차별점이라 **기본 상태에서 보여야 한다**.

            **카드 경계는 그대로 독립 L1 이다** (§0 "카드 경계는 이야기 단위"). 기본 정보
            카드 안에 넣으면 `주소·전화·운영시간` 이라는 절 이름 아래 판정성 카드가 들어가
