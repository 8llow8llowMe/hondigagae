...
export function useAiPlanJob(jobId: string) {
  ...
  */
  ...
  return {
    ...
    phase,
    /**
     * 구독을 시작한 뒤 지난 시간(ms) (#710).
     *
     * **이 값은 지금까지 훅 밖으로 나가지 않았다.** `JOB_POLL_LIMIT_MS` 주석이 "tick 에 매인
     * 리렌더가 최악 330회" 를 감수한다고 적어 두고 있었는데, 그 리렌더가 만들어 내는 화면
     * 변화는 국면이 바뀌는 두 순간(90초 · 330초)뿐이었다 — 나머지 328번은 아무것도 바꾸지
