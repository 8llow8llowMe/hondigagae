 ...
 *
 * ### 여기에 없는 것 — `Canvas` 바닥 높이
 *
 * 상태 파일은 대개 한 화면을 못 채워 L0 회색이 콘텐츠 높이에서 끊기고 그 아래로 흰
 * 페이지가 보인다. **`min-h` 를 여기서 주지 않는다** — 레이아웃에 한 번 거는 규칙이라
 * **#456③** 의 소관이고, 이미 3a 인 화면들(짧은 `/pets` · 빈 `/favorites`)도 같은 증상이라
 * 이 이슈가 만든 문제가 아니다. 상태 파일에만 `min-h` 를 달면 같은 규칙이 두 곳으로
 * 갈린다. (#475 본문이 "겹치면 #456 을 먼저 하는 편이 나을 수 있다 — 착수 전에 판단해라"
 * 를 요구한 것에 대한 답이 이 문단이다.)
 */
