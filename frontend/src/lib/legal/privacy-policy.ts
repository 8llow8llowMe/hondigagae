 ...
 *  - 탈퇴·파기: `backend/docs/services/auth-service.md` 의 "탈퇴" 절 (#609 · #688)
 *
 * **이 사실들이 바뀌면 문서를 먼저 고친다.** 좌표를 저장하는 기능이 생기거나 외부
 * LLM 으로 갈아타면 제2조·제3조·제5조·제13조·제14조가 즉시 거짓이 된다.
 *
 * **`version` 을 올리지 않았다** (#688). 백엔드가 `legal.privacy-version` 설정으로 같은
 * 값을 따라 적고 동의 이력에 그 값을 남기므로, 상수만 올리면 화면은 새 판을 보여주고
 * 이력에는 옛 판이 남는다. 두 값은 같은 배포로 함께 올려야 한다 — 올릴지 여부는 백엔드와
 * 맞춘 뒤 결정한다.
 */
  ...
  articles: [
      ...
      blocks: [
        {
          ...
          headers: ['구분', '보유 기간'],
          rows: [
            ['회원 정보 (이메일, 비밀번호, 이름, 닉네임, 프로필 이미지)', '회원 탈퇴 시까지'],
            ['반려견 프로필', '회원 탈퇴 후 30일'],
            ['동의 이력 (동의한 문서와 그 판, 만 14세 이상 확인)', '회원 탈퇴 후 30일'],
            ['탈퇴 회원의 이메일 (일방향 값으로 치환, 재가입 제한 목적)', '회원 탈퇴 후 30일'],
            ['여행 일정, 저장한 장소', '탈퇴 후에도 보존 (제6조 참고)'],
        ...
        {
          kind: 'text',
          text: '탈퇴 시 이름과 닉네임은 마스킹하고 비밀번호와 프로필 이미지는 삭제하며, 이메일은 원본을 남기지 않고 복원할 수 없는 일방향 값으로 치환합니다. 이 값은 같은 계정이 다시 가입하는지 판정하는 목적으로만 보관하며 그 밖의 용도로 이용하지 않습니다. 파기 절차는 제6조에 있습니다.',
        },
      ],
    },
    {
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            [
              '소셜 로그인(필수)',
              '소셜 계정 이메일, 소셜 서비스 식별자',
              '카카오·네이버 로그인 시 이용자 동의를 거쳐 전달받음',
            ],
            [
              '가입 동의 기록(필수)',
              '이용약관·개인정보 처리방침 동의 여부와 동의 시각, 동의한 문서의 판, 만 14세 이상 확인 여부',
              '회원가입 시 회원 직접 선택',
            ],
    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'text', text: '회원 탈퇴의 경우 다음 순서로 파기합니다.' },
        {
          kind: 'table',
          headers: ['시점', '처리 내용'],
          rows: [
            [
              '탈퇴 즉시',
              '이메일을 복원할 수 없는 일방향 값으로 치환하고(원본은 남기지 않습니다) 비밀번호와 프로필 이미지를 삭제하며 이름과 닉네임을 마스킹합니다',
            ],
            [
              '탈퇴 후 30일',
              '회원 정보, 반려견 프로필, 동의 이력을 삭제합니다. 이 시점부터 같은 이메일로 다시 가입할 수 있습니다',
            ],
          ],
        },
        {
          kind: 'text',
          text: '탈퇴 후 30일 동안 이메일의 일방향 값을 보관하는 것은 착오로 탈퇴한 계정이 곧바로 다시 가입하는 것을 막기 위한 것입니다. 같은 이메일인지 판정하는 데에는 원본이 필요하지 않으므로 원본은 보관하지 않으며, 이 값을 그 밖의 용도로 이용하지 않습니다.',
        },
        {
          kind: 'text',
          text: '다만 회원이 만든 여행 일정과 그 일정에 담긴 반려견 특성 요약은 탈퇴 후에도 남습니다. 이 기록에는 이메일·이름·연락처와 같이 개인을 알아볼 수 있는 항목이 없으며, 탈퇴 후 30일이 지나면 이 기록과 연결할 수 있는 회원 정보도 존재하지 않습니다.',
        },
      ],
    },
    {
