 ...
 * 인데, **그 문제는 여기서도 그대로 일어난다** — 밀도가 달라서 피해가는 것이 아니다.
 * 반경 10km 안에 136곳이라도 4(1280 폭 2.6km · 375 폭 750m)로 확대하면 그 좁은 프레임
 * 안에는 몇 곳밖에 남지 않는다. 브라우저 실측: 레벨 4 로 확대했더니 136곳 중 4곳만
 * 프레임 안에 남았다.
 *
 * **그런데도 4 를 쓸 수 있는 이유는 밀도가 아니라 `emergency-map-view.tsx` 의
 * `frozenBounds` 다.** 선택이 살아 있는 동안은 목록·칩이 세는 영역을 선택 **직전**
 * 값으로 얼려 두므로, 지도가 확대돼도 목록은 줄지 않는다. **`frozenBounds` 를
 * 없애면 이 값은 다시 4가 아니라 5여야 한다** — 이 상수를 옮기거나 얼림을 걷을
 * 때는 반드시 함께 본다.
 *
