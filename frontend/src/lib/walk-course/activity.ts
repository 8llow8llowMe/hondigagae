 ...
 *
 * **상한은 서버가 갖는다.** `WalkCourseActivityFit.fits` 가 거르고, 소요시간을 모르는
 * 코스(`durationMaxMinutes === null`)는 **어느 활동량에서도 걸러지지 않는다.** FE 는 다시
 * 계산하지 않는다.
 *
 * **상한 숫자도 이 파일에 없다** (#735). 목록 응답의 `appliedPetActivityLevel.maxDurationMinutes`
 * 가 적용된 상한을 내려주므로 화면은 그 값만 말한다 — 여기 `4`·`6` 을 복제해 두면 서버가
 * 상한을 바꾼 날 화면만 옛 숫자를 말한다.
 *
 ...
 * `appliedPetActivityLevel` 만 채워져(규칙 3), 후자는 400(`WALKCOURSE_113`)이 된다.
 */
