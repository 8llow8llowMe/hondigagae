...
export const aiPlanMessages = {
  ...
  */
  ...
  /**
   * 경과 시간 줄 (#710). `{elapsed}` 치환 — `1분 12초` 처럼 들어온다.
   *
   * **`4 / 4단계` 하나로는 기다리는 구간에서 화면이 아무 말도 하지 않는다.** 네 단계 중
   * `DRAFTING`(LLM 호출)이 전체의 90% 이상이라 사람이 보는 거의 모든 시간이 마지막 한
   * 칸이고, 그 동안 숫자가 멈춰 있다. 지난 시간은 관측값이라 지어낸 진행률이 아니면서
   * **작업이 살아 있다는 것을 1초마다 증명한다.**
   *
   * **`1~2분쯤` 은 조건 입력 화면의 `createSubmitHint` 와 같은 약속이다.** 거기서 한 번
   * 말하고 정작 기다리는 화면에서 사라져 있었다 — 기대치를 세운 자리와 그것을 확인해야
   * 하는 자리가 갈려 있으면 약속이 없는 것과 같다.
   */
  jobElapsed: '{elapsed} 지남 · 보통 1~2분쯤 걸려요',
  /**
   * 조건 블록 라벨 — 기다리는 중 (#710).
   *
   * **진행 화면에도 조건을 그린다.** 서버가 생성 조건을 함께 내려주므로(#488) 값은 이미
   * 화면에 있었는데 실패·취소에만 쓰고 있었다. 기다리는 동안이야말로 "지금 뭘 만들고
   * 있나" 가 궁금한 시점이다.
   */
  jobConditionLabel: '만드는 조건',
  /**
  ...
  failedChange: '조건 바꾸기',
  failedManual: 'AI 없이 직접 만들기',
  /**
   * 조건 블록 라벨 — 끝난 뒤(실패·취소) (#710).
   *
   * **라벨이 곧 안내다.** 예전에는 `입력한 조건({summary})은 그대로 남아 있어요.` 한
   * 문장이 **버튼 아래** caption 으로 있었는데, 순서가 거꾸로였다 — "무엇이 남아 있나"
   * 를 읽어야 "그래서 다시 시도할 수 있다" 가 이해된다. 조건 블록을 버튼 **위**로 올리고
   * 문장을 라벨로 줄이면 괄호 안에 갇혀 있던 조건이 제 줄을 갖는다.
   */
  jobConditionKeptLabel: '그대로 남아 있는 조건',
  /**
   * 제한 시간 초과(`AIPLAN_006`)일 때 덧붙이는 단서 (#710 · #711).
   *
   * **서버 문구는 무슨 일이 났는지까지만 말한다** — `AI 일정 생성이 제한 시간을
   * 넘겼습니다.` 를 읽은 사용자가 할 수 있는 일이 무엇인지는 말하지 않는다. 기간이 길수록
   * LLM 이 짜야 하는 일자가 늘어 오래 걸리므로, 줄이는 것이 실제로 듣는 수다.
   *
   * `failedNarrowedRegion`(#251)과 같은 자리·같은 규칙이다 — 서버 문구를 **대신하지 않고
   * 아래에 덧붙인다.**
   */
  failedTimeoutHint: '여행 기간을 하루 줄이면 더 빨리 만들어져요.',
  /**
  ...
  commitDelistedDescription:
    '아래 표시한 곳은 더 이상 조회되지 않아요. 그 항목을 빼면 나머지는 담을 수 있어요.',
