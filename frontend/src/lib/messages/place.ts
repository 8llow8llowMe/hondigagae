...
export const placeMessages = {
  ...
  /**
   * 체감온도 계산 근거의 펼침 라벨 (#292).
   *
   * **서버 `feelsLikeBasis` 를 접어 둔다.** 산식·입력·임계 출처를 다 담은 130자 문장이라
   * 펼쳐 두면 판정(hero)과 근거 목록 사이에 회색 벽이 선다. 그러나 감추지도 않는다 —
   * 노면온도는 라벨이 `추정` 이라고 스스로 밝히는데 hero 체감온도만 근거가 없었다.
   *
   * **참고 열지수가 이 안에 함께 산다.** 서버는 `heatIndexCelsius` 를 계속 내려주지만
   * 판정에 쓰지 않으므로 평면에 세 번째 온도로 세우지 않는다 — 그러면 판정값과 참고값이
   * 같은 위계가 된다. 값과 "판정에는 쓰지 않는다" 는 문장(`heatIndexBasis`)을 **한자리에
   * 두어** 숫자만 떼어 읽히는 일을 막는다.
   */
  detailFeelsLikeBasisOpen: '체감온도 계산 근거 보기',
  detailFeelsLikeBasisClose: '계산 근거 접기',

  /**
   * 펼친 근거 안의 참고 열지수 라벨 (#292).
   *
