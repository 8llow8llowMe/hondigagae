...
export const petMessages = {
   ...
   * (`styling-guide.md` §3-2 의 "이미 끝났다"), 저장 직후 `/pets` 로 넘어가므로
   * 폼 화면에는 결과를 남길 자리가 없다.
   *
   * 조사는 `withObjectParticle` 이 붙인다 — 이름이 "몽실이"(종성 없음)일 수도
   * "곰"(종성 있음)일 수도 있어 고정하면 한쪽이 반드시 틀린다.
   */
  registeredToast: '{name} 등록했어요',

  /**
   * 제출 실패 요약 — `{count}` 치환. 화면 밖 필드가 틀렸을 때 **무엇이 잘못됐는지
   * 모르는 것**을 닫는다 (#538).
   *
   * **토스트가 아니라 `FormAlert` 로 낸다.** `styling-guide.md` §3-2 가 "오류를
   * 토스트로 말하지 않는다 — 오류는 섹션 안에 남아야 다시 시도할 수 있다" 를 규칙으로
   * 박아 뒀다. 사라지는 띠에 담으면 스크롤해 내려간 뒤 개수를 다시 볼 수 없다.
   *
   * **"입력하지 않은" 이라고 쓰지 않는다.** 크기·활동량·사회성은 초기값이 이미 있어
   * (`EMPTY_PET_FORM_VALUES`) 미입력으로 실패하지 않고, 실제로 걸리는 것은 생년월
   * 형식(PET_104) · 체중 범위(PET_108) · 체중↔크기 모순(PET_004) 처럼 **채웠는데
   * 어긋난** 값이다. 미입력이라고 말하면 채운 사람이 자기 오류를 못 찾는다.
   */
  submitInvalidSummary: '확인이 필요한 항목이 {count}개 있어요',
