...
describe('권역 칸 — 폭 산식 (#412 · #530)', () => {
  ...
  */
   ...
   *
   * **`100` 이던 배지가 `100점` 이 됐다.** 한글 한 글자는 `text-caption`(12px)에서 10.4px
   * 이다 (390 실측 — `100점` 59.5 에서 `100` 49.1 을 뺀 값).
   *
   * **문구에서 읽는다.** 단위가 두 글자로 늘면 산식이 따라 움직여야 하는데, 숫자를 여기
   * 박아 두면 그때 조용히 어긋난다.
   */
  function unitWidth(): number {
    return [...messages.home.regionScoreUnit.replace('{score}', '')].length * 10.4
  }
