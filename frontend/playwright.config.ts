...
export default defineConfig({
  ...
  webServer: {
    ...
    */
    ...
    url: BASE_URL,
    /*
      ### `... is already used` 를 보고 여기 왔다면

      **띄워 둔 서버를 그대로 쓰려면 `E2E_REUSE_SERVER=1`**, 다른 포트로 돌리려면
      `E2E_PORT=<빈 포트>`. 서버를 죽일 필요는 없다. playwright 의 기본 실패 문구는
      이 옵트인을 알려 주지 않으므로 여기 먼저 적는다.

      ### 왜 기본이 꺼져 있나 (#773)

      **원래 의도는 바뀌지 않았다.** 이 자리의 옛 주석도 "남의 세션 서버를 재사용하지
      않는다" 였고, 포트를 갈라 두는 것(사람은 5174)으로 그것을 지키려 했다. 워크트리를
      나눠 쓰는 **병렬 세션이 그 뒤에 생기면서 전제가 깨졌을 뿐**이라, 기본을 끈 것은
      새 정책이 아니라 **같은 의도를 수단만 바꿔 지키는 것**이다.

      재사용이 켜져 있으면 그 포트에 이미 뜬 서버를 줍는데, 그것이 **남의 코드를 검사하고
      통과하는** 길이 된다 — 초록불이 내 변경을 증명하지 못한다. 끄면 포트가 잡혀 있을 때
      **시끄럽게 실패한다.** 조용히 틀린 통과보다 낫다.

      옵트인으로 켰을 때는 그 포트의 서버가 **내 코드**인지 본인이 책임진다.
