 ...
 */
/**
 * **`/privacy` 는 3a 에 가장 늦게 들어온 화면이다.** #610 이 만든 약관 두 장은 로드맵
 * #455 의 이동 목록에서 빠져 있어, 제품에서 **본문이 카드 밖에 있는 유일한 화면**으로
 * 남아 있었다. 여기 넣어 그 자리로 되돌아가지 않게 잠근다.
 *
 * **미로그인으로도 열리는 유일한 화면이기도 하다** — 목 세션이 깨져도 이 행은 남는다.
 * 약관과 처리방침은 같은 렌더러(`LegalDocumentView`)라 한 장만 잰다.
 */
const SCREENS = [
  '/mypage',
  '/pets',
...
test.describe('제목이 필터보다 먼저다 — #546', () => {
  test('데스크톱 개요가 h1 → h2 필터 → h3 셋 이다', async ({ page }) => {
    ...
    /*
      ...
      **축 순서는 `EmergencyFilterRail` 이 정한다** — 이 단언이 재는 것은 `h1` 이 `h2 필터`
      **앞**에 오는가(#546)이지 축의 순서가 아니다. #654(감사 E-3)가 급한 순서로 뒤집어
      `영업 조건`이 맨 앞이 됐다: 이 화면을 여는 사람이 처음 찾는 것은 «지금 진료중» 이고,
      반경은 결과를 보고 부족하다고 판단한 뒤에 찾는다. 셋을 그대로 적어 두는 것은
      **레일에서 축이 조용히 사라지는 것**을 잡기 위해서다.
    */
