...
- 좌표를 **숫자로 안전하게 파싱하는가** — TourAPI `mapx`(경도)/`mapy`(위도)는 문자열로 온다
- **경도/위도 순서를 뒤집지 않았는가** — 카카오는 `new kakao.maps.LatLng(위도, 경도)` 순서다. `mapy`(위도)가 첫 인자여야 한다
- 좌표가 `null`/`0`/빈 문자열인 장소를 마커로 그리려 하는가 → 지도가 아프리카 앞바다로 튄다
