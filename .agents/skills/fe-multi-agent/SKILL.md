...
5. **구현과 검토를 진행한다.**
   ...
   - 각 역할에는 `.claude/agents/fe-*.md`의 프로젝트 정의를 역할 프롬프트로 제공한다
   - 독립적인 읽기 전용 검토는 병렬로 실행할 수 있다
   - 구현은 메인 실행자 또는 단일 `fe-implementer`만 맡아 편집 충돌을 막는다
...
- 호스트별 도구 이름이나 특정 내장 agent 이름을 전제로 삼지 않는다.
