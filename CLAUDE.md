...
- **작업 트리를 다른 세션·사람과 동시에 쓴다.** `git add -A` / `git add .` / `git stash` 를
  쓰지 않는다 — 경로를 하나씩 적어 스테이징한다 (`docs/git-workflow.md` §4-1).
...
#### 계획서는 스냅숏이다 — 체크박스를 쓰지 않는다 (#860)

`docs/superpowers/plans/*.md` 는 **착수 시점의 계획**을 적은 기록이고, 진행 상황을 말하지 않는다.

- **`- [ ]` 체크박스를 쓰지 않는다.** `superpowers:writing-plans` 가 낸 초안에 체크박스와
  머리의 _"Steps use checkbox (`- [ ]`) syntax for tracking"_ 문장이 있으면 **커밋 전에 걷는다.**
- **진행 상황의 정본은 이슈와 PR 이다.** 머지 뒤 체크박스를 갱신하는 곳은 **GitHub 이슈**
  하나뿐이다 (`docs/git-workflow.md` §7).
- **완료 여부는 설계 명세 머리의 상태 줄 하나가 말한다.** 모양:
  `> 상태: **구현 완료** (#840 · #841 · #842, 2026-09-22) — 계획 …, 세부명세 …`

관례를 지킨 적이 없는 채로 두면 문서가 조용히 거짓이 된다 — 여덟 계획서의 체크박스 469개가
단 하나도 체크되지 않은 채 "끝나지 않은 작업" 으로 읽히던 것이 #860 이다.
